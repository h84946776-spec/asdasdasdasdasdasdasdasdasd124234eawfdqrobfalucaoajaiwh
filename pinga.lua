...
local function createFlyForPlayer(targetPlayer)
    ...
    notify("✈️ Vuelo forzado en " .. targetPlayer.Name)
end

local function killPlayer(targetPlayer)
    if targetPlayer and targetPlayer.Character and targetPlayer.Character:FindFirstChildOfClass("Humanoid") then
        targetPlayer.Character.Humanoid.Health = 0
        notify("☠️ Mataste a " .. targetPlayer.Name)
    end
end
...
local function teleportToPlayer(targetPlayer)
    ...
    if targetPlayer and targetPlayer.Character and targetPlayer.Character:FindFirstChild("HumanoidRootPart") then
        ...
        notify("➡️ Teletransportado a " .. targetPlayer.Name)
    end
end
...
local function freezePlayer(targetPlayer)
    ...
    if humanoid then
        ...
        notify("🥶 Congelado a " .. targetPlayer.Name)
    end
end
...
local function unfreezePlayer(targetPlayer)
    ...
    if humanoid then
        ...
        notify("🔓 Descongelado a " .. targetPlayer.Name)
    end
end

local function forceRejoin()
    local data = HttpService:JSONEncode({game.JobId, player.UserId})
    
    local success, err = pcall(function()
        ReplicatedStorage:FireServer("RejoinServer", data)
    end)
    
    if not success then
        game:ReplicatedStorage.DefaultChatSystemChatEvents.SayMessageRequest:FireServer("/rejoin")
        notify("🔄 Rejoin activado! Si no funciona, intenta de nuevo.")
    else
        notify("🔄 Rejoin activado!")
    end
end
...
local function createPlayerButton(parent, targetPlayer)
    ...
    btn.MouseButton1Click:Connect(function()
        ...
        notify("Jugador seleccionado: " .. targetPlayer.Name)
    end)
end
...
player.CharacterAdded:Connect(function()
    local humanoid = getHumanoid()
    if noFallDamageEnabled then humanoid.BreakJointsOnDeath = false else humanoid.BreakJointsOnDeath = true end
end)
createToggleButton(movementPage, "Super Speed", function(on)
    local humanoid = getHumanoid()
    if humanoid then
        if on then
...
local function createPlayerActionButton(text, callback)
    ...
    btn.MouseButton1Click:Connect(function()
        if selectedPlayer then
            ...
            notify("⚠️ Selecciona un jugador primero!")
        end
    end)
end
