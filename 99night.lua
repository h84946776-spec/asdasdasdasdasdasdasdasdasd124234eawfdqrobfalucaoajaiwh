--[[
SETIP HUB
SETIP HUB 99 NIGHT IN THE FOREST
----------------------------------------------------------------------------
  IMPORTANT:
...
if game.PlaceId == 79546208627805 then
    pcall(function()
        game:GetService("StarterGui"):SetCore("SendNotification", {
            Title = "Setip Hub 🪽,"
            Text = "Go In Game for SETIP to load :D [You are in lobby currently]",
            Duration = 10
        })
    end)
    return
                    ...
                    if TextSource then
                        ...
                        if plr then
                            ...
                            if currentLevel then
                                prefix = prefix .. string.format("<font color='rgba(0, 180, 240, 1)'>[</font><font color='rgb(255,255,255)'>%s</font><font color='rgb(173,216,230)'>]</font> ", tostring(currentLevel))
                            end
                            local playerTagValue = plr:FindFirstChild("PlayerTagValue")
                            if playerTagValue and playerTagValue.Value then
                                prefix = prefix .. string.format("<font color='rgba(0, 3, 1, 1)'>[</font><font color='rgb(255,255,255)'>#%s</font><font color='rgb(173,216,230)'>]</font> ", tostring(playerTagValue.Value))
                            end
                            prefix = prefix .. PrefixText
                            properties.PrefixText = string.format("<font color='rgba(236, 236, 236, 1)'>%s</font>", prefix)
                        end
                    end
                    return properties
                end
